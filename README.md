...
If you are upgrading from a previous version (e.g. 8.11.3) then skip to the [upgrading](#upgrading-existing-installations) section below. Replace the `XXXXXX` attachment ID in the fetch URL below with the latest version available on the [Bugzilla page](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=288653).

```shell
mkdir /usr/local/ports
cd /usr/local/ports
fetch "https://bugs.freebsd.org/bugzilla/attachment.cgi?id=XXXXXX" -o /usr/local/ports/elasticsearch-8.19.tar.gz
tar xvf elasticsearch-8.19.tar.gz
...
Replace the `XXXXXX` attachment ID in the fetch URL below with the latest version available on the [Bugzilla page](https://bugs.freebsd.org/bugzilla/show_bug.cgi?id=288653).
